...
export default function UpcomingBirthdays() {
  ...
  return (
    <>
      <SpotlightCard className="p-6 w-full" spotlightColor="rgba(44, 121, 255, 0.15)">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
              🎂 다가오는 생일
            </h2>
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">이번 달 생일을 맞이하는 사람들</p>
          </div>
          <button
            onClick={() => setIsModalOpen(true)}
            className="px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 text-xs font-semibold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors whitespace-nowrap"
          >
            생일 모두 보기
          </button>
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col gap-3">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-800 animate-pulse">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-6">다가오는 생일이 없습니다</p>
        ) : (
          <div className="flex flex-col gap-3">
            {entries.map((entry) => (
              <div
                key={`${entry.name}-${entry.month}-${entry.day}`}
                className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500">{entry.label}</p>
                </div>
